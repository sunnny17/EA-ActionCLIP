...
class Action_DATASETS(data.Dataset):
    def __init__(self, list_file, labels_file, root,
    ...
    def _load_image(self, directory, idx):
        ...
        img_path = os.path.join(full_dir, self.image_tmpl.format(idx))
    ...
    def __getitem__(self, index):
        ...
        #print(f"[DEBUG] index={index}, path={record.path}, num_frames={record.num_frames}")
        segment_indices = self._sample_indices(record) if self.random_shift else self._get_val_indices(record)
        #print(f"[DEBUG] sampled indices: {segment_indices}")
